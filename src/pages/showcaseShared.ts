 ...
 * 详页与看板一样支持多数据源切换：数据源下拉列出注册表全部源，
 * 标的随数据源固定为该源默认（与 Dashboard 的 SOURCE_DEFAULTS 保持一致），
 * 周期下拉按该源支持的周期动态过滤。
 *
 * 注意：东方财富在当前网络被 TLS 阻断（见 eastmoney.ts 注释），选择它会显示
 * error-panel 优雅报错；腾讯财经（默认源）与 Binance 可正常直连。
 */
export const SHOWCASE_SOURCE_DEFAULTS: Record<DataSourceId, { symbol: string; label: string }> = {
  binance: { symbol: 'BTCUSDT', label: 'BTCUSDT' },
  tencent: { symbol: 'sh000001', label: '上证指数' },
  eastmoney: { symbol: 'sh000001', label: '上证指数' },
  tdx: { symbol: 'sh000001', label: '上证指数' },
};
